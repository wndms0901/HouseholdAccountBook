...
## Spring Data JPA
Spring Data JPA는 JPA를 한단계 추상화 시킨 Repository 인터페이스를 제공하여 JPA를 편리하게 사용할 수 있도록 해주는 모듈이다.   
<br>
<img src="https://user-images.githubusercontent.com/53942595/125742823-6a2d9749-7650-430c-8c21-9fb1dd080673.JPG">
* ### **JPA(Java Persistence API)**   
  * 자바 ORM 기술에 대한 API 표준 명세를 의미한다.
  * 인터페이스인 JPA를 사용하기 위해서 구현체가 필요하며 대표적으로 Hibernate, Eclipse LInk 등이 있다.
  * 스프링 부트에서는 기본적으로 Hibernate를 JPA구현체로 이용한다.
  * ORM(Object-Relational Mapping) : 객체(Object)와 관계형 데이터베이스(Relational Database)의 데이터를 매핑(Mapping)해주는 것을 의미한다. 객체 간의 관계를 바탕으로 SQL을 자동으로 생성해서 sql쿼리문 없이도 데이터베이스의 데이터들을 다룰 수 있다.
* ### **JPA 장점**   
  * CRUD 쿼리문을 직접 작성할 필요가 없음
  * 생성한 객체로 데이터베이스를 다루기 때문에 객체 지향 개발이 가능함
  * 데이터베이스에 종속적이지 않으므로 특정 쿼리를 사용하지 않아 추상적으로 기술 구현이 가능함
* ### **JPA 단점**   
  * 복잡한 쿼리문 작성이 어려움
  * 학습곡선이 높음
## CI/CD
* ### **CI(Continuous Integration)** 
  * 코드 버전 관리를 하는  VCS 시스템(Git, SVN 등)에 PUSH가 되면 자동으로 테스트와 빌드가 수행되어 안정적인 배포 파일을 만드는 과정으로 지속적 통합을 의미한다.
* ### **CD(Continudous Deployment)** 
  * 빌드 결과를 자동으로 배포하는 것으로 지속적 배포를 의미한다.
* ### **CI/CD 장점** 
  * 빌드, 테스트, 배포 자동화로 서비스의 안정성을 확보할 수 있음
  * 요구사항, 버그에 대해 유연하고 빠르게 대처할 수 있음
  * 개발자들의 생산성을 높일 수 있음
* ### **CI/CD Tools** 
  * Jenkins, CircleCI, TeamCity, Travis-CI 등이 있다.
